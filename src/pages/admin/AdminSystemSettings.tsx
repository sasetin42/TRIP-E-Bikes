...
import { 
  ...
  Globe, Mail, Lock, Key, ShieldAlert, Upload, Image, Trash2, CheckCircle2
} from "lucide-react";
import { toast } from "sonner";
import { db, storage } from "@/lib/firebase";
import { doc, getDocs, setDoc, collection, Timestamp } from "firebase/firestore";
...
export default function AdminSystemSettings() {
  ...
  const [smtpPass, setSmtpPass] = useState("re_abc123xyz...");
  ...
  const loadSettings = useCallback(async () => {
    ...
    try {
      ...
      snap.forEach((docSnap) => {
        ...
        // Route keys into appropriate states
        if (k.endsWith("_enabled")) {
          featureList.push({
            key: k,
            value: val === true || val === "true" || val === "1",
            label: k.replace(/_/g, " ").replace("enabled", ""),
            description: `Toggle status of ${k.replace(/_/g, " ")} feature`
          });
        } else {
          switch (k) {
            case "site_title": setSiteTitle(String(val)); break;
            case "support_email": setSupportEmail(String(val)); break;
            case "support_phone": setSupportPhone(String(val)); break;
            case "currency": setCurrency(String(val)); break;
            case "system_status": setSystemStatus(String(val)); break;
            case "logo_main": setLogoMain(String(val)); break;
            case "logo_dark": setLogoDark(String(val)); break;
            case "logo_favicon": setLogoFavicon(String(val)); break;
            case "smtp_host": setSmtpHost(String(val)); break;
            case "smtp_port": setSmtpPort(String(val)); break;
            case "smtp_user": setSmtpUser(String(val)); break;
            case "smtp_pass": setSmtpPass(String(val)); break;
            case "smtp_encrypt": setSmtpEncrypt(String(val)); break;
            case "smtp_from_email": setSmtpFromEmail(String(val)); break;
            case "smtp_from_name": setSmtpFromName(String(val)); break;
            case "security_min_password": setMinPasswordLength(String(val)); break;
            case "security_require_special": setRequireSpecialChar(val === true || val === "true" || val === "1"); break;
            case "security_mfa_required": setTwoFactorRequired(val === true || val === "true" || val === "1"); break;
            case "security_session_timeout": setSessionTimeout(String(val)); break;
            case "security_ip_lockout": setIpLockout(String(val)); break;
          }
        }
      });

      // Default fallback variables if db is empty
  ...
  const toggleFeature = async (key: string, currentValue: boolean) => {
    ...
    try {
      await setDoc(doc(db, "system_settings", key), {
  ...
  const handleAssetUpload = async (key: string, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const storageRef = ref(storage, `brand/${key}`);
  ...
  const handleAssetDelete = async (key: string) => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Brand asset not found in storage:", e);
      }
    ...
    <div className="space-y-8 max-w-5xl">
        ...
        <div className="glass rounded-2xl border border-white/5 p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-1.5 md:col-span-2">
              ...
              <select
                ...
                className="w-full bg-[#121212] border border-white/10 rounded-xl px-4 py-3 text-xs text-white focus:outline-none focus:border-[#39FF14]/30"
              >
          ...
          <div className="flex justify-end pt-4">
            <button
              onClick={saveSection}
              disabled={saving}
              className="px-6 py-3 rounded-xl bg-[#39FF14] text-[#0A0A0A] hover:bg-[#4FFF2A] text-xs font-black uppercase tracking-wider transition-all flex items-center gap-1.5 disabled:opacity-50"
            >
      ...
      {activeTab === "appearance" && (
        <div className="glass rounded-2xl border border-white/5 p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* Asset card: Main Logo */}
            <div className="p-4 rounded-xl border border-white/5 bg-white/2 flex flex-col items-center justify-between text-center space-y-4">
              <span className="text-[11px] font-bold text-gray-400 font-orbitron uppercase tracking-wide">Main Logo</span>
              <div className="w-32 h-20 border border-white/10 rounded-lg flex items-center justify-center bg-black overflow-hidden relative">
                {logoMain ? (
                  <img src={logoMain} alt="Main Logo" className="max-h-full max-w-full object-contain" />
                ) : (
                  <span className="text-[10px] text-gray-600">No main logo</span>
                )}
                {uploadingLogo === "logo_main" && (
              ...
              <div className="flex gap-2">
                <label className="px-2.5 py-1.5 rounded-lg border border-white/10 bg-white/5 hover:bg-white/10 text-[10px] font-semibold text-gray-300 hover:text-white cursor-pointer transition-all flex items-center gap-1">
                  ...
                  <input type="file" accept="image/*" className="hidden" onChange={(e) => handleAssetUpload("logo_main", e)} />
                </label>
                {logoMain && (
                  <button
                    onClick={() => handleAssetDelete("logo_main")}
            ...
            {/* Asset card: Dark Mode Logo */}
            <div className="p-4 rounded-xl border border-white/5 bg-white/2 flex flex-col items-center justify-between text-center space-y-4">
              <span className="text-[11px] font-bold text-gray-400 font-orbitron uppercase tracking-wide">Dark Mode Logo</span>
              <div className="w-32 h-20 border border-white/10 rounded-lg flex items-center justify-center bg-white overflow-hidden relative">
                {logoDark ? (
                  <img src={logoDark} alt="Dark Logo" className="max-h-full max-w-full object-contain" />
                ) : (
                  <span className="text-[10px] text-gray-400">No dark logo</span>
                )}
              ...
              <div className="flex gap-2">
                <label className="px-2.5 py-1.5 rounded-lg border border-white/10 bg-white/5 hover:bg-white/10 text-[10px] font-semibold text-gray-300 hover:text-white cursor-pointer transition-all flex items-center gap-1">
                  <Upload className="w-3.5 h-3.5" /> Upload
                  <input type="file" accept="image/*" className="hidden" onChange={(e) => handleAssetUpload("logo_dark", e)} />
                </label>
                {logoDark && (
                  <button
                    onClick={() => handleAssetDelete("logo_dark")}
                    className="p-1.5 rounded-lg border border-red-500/20 bg-red-500/5 hover:bg-red-500/10 text-red-400"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
                  </button>
            ...
            {/* Asset card: Favicon */}
            <div className="p-4 rounded-xl border border-white/5 bg-white/2 flex flex-col items-center justify-between text-center space-y-4">
              <span className="text-[11px] font-bold text-gray-400 font-orbitron uppercase tracking-wide">Favicon Shortcut</span>
              <div className="w-32 h-20 border border-white/10 rounded-lg flex items-center justify-center bg-black overflow-hidden relative">
                {logoFavicon ? (
                  <img src={logoFavicon} alt="Favicon" className="w-8 h-8 object-contain" />
                ) : (
                  <span className="text-[10px] text-gray-600">No favicon</span>
                )}
                {uploadingLogo === "logo_favicon" && (
                  <div className="absolute inset-0 bg-black/80 flex items-center justify-center">
                    <Loader2 className="w-5 h-5 text-[#39FF14] animate-spin" />
                  </div>
                )}
              ...
              <div className="flex gap-2">
                <label className="px-2.5 py-1.5 rounded-lg border border-white/10 bg-white/5 hover:bg-white/10 text-[10px] font-semibold text-gray-300 hover:text-white cursor-pointer transition-all flex items-center gap-1">
                  <Upload className="w-3.5 h-3.5" /> Upload
                  <input type="file" accept="image/*" className="hidden" onChange={(e) => handleAssetUpload("logo_favicon", e)} />
                </label>
                {logoFavicon && (
                  <button
                    onClick={() => handleAssetDelete("logo_favicon")}
                    className="p-1.5 rounded-lg border border-red-500/20 bg-red-500/5 hover:bg-red-500/10 text-red-400"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
                  </button>
      ...
      {activeTab === "smtp" && (
        <div className="glass rounded-2xl border border-white/5 p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-1.5">
              <label className="text-xs text-gray-400 font-semibold">SMTP Host Address</label>
              <input
                type="text"
                value={smtpHost}
            ...
            <div className="space-y-1.5">
              <label className="text-xs text-gray-400 font-semibold">SMTP Port Number</label>
              <input
                type="text"
                value={smtpPort}
            ...
            <div className="space-y-1.5">
              <label className="text-xs text-gray-400 font-semibold">SMTP Login Username</label>
              <input
                type="text"
                value={smtpUser}
            ...
            <div className="space-y-1.5">
              <label className="text-xs text-gray-400 font-semibold">SMTP Login Password</label>
              <input
                type="password"
                value={smtpPass}
            ...
            <div className="space-y-1.5">
              <label className="text-xs text-gray-400 font-semibold">Sender Name (From Name)</label>
              <input
                type="text"
                value={smtpFromName}
            ...
            <div className="space-y-1.5">
              <label className="text-xs text-gray-400 font-semibold">Sender Email (From Address)</label>
              <input
                type="email"
                value={smtpFromEmail}
            ...
            <div className="space-y-1.5 md:col-span-2">
              ...
              <select
                value={smtpEncrypt}
                onChange={(e) => setSmtpEncrypt(e.target.value)}
                className="w-full bg-[#121212] border border-white/10 rounded-xl px-4 py-3 text-xs text-white focus:outline-none focus:border-[#39FF14]/30"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-1.5 md:col-span-2">
              ...
              <input
                type="text"
                value={ipLockout}
                onChange={(e) => setIpLockout(e.target.value)}
                placeholder="e.g. 192.168.1.1, 10.0.0.1/24 (Leave empty to allow all IP addresses)"
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-xs text-white focus:outline-none focus:border-[#39FF14]/30"
              />
            </div>
          </div>
